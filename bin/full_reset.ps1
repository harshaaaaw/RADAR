...
Get-ChildItem -Path "$PSScriptRoot\src" -Recurse -Directory -Filter "__pycache__" -ErrorAction SilentlyContinue | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
Write-Host "  Done" -ForegroundColor Green
...
$queueDb = "$PSScriptRoot\runtime\queue\queues.db"
