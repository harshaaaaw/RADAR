...
# All dependencies are bundled inside the bin/ folder - no external installs needed
# ============================================================================
$ProjectRoot    = Split-Path -Path $PSScriptRoot -Parent
$BinDir         = $PSScriptRoot

# Derive all dependency paths from $BinDir (bundled in bin/)
$NssmPath       = "$BinDir\nssm-2.14\win64\nssm.exe"
$RedisPath      = "$BinDir\Redis-x64-3.2.100\redis-server.exe"
$OpenSearchBin  = "$BinDir\opensearch-2.12.0\bin\opensearch.bat"
$OpenSearchHome = "$BinDir\opensearch-2.12.0"
$TikaJar        = "$BinDir\tika\tika-server-2.9.2.jar"
$JavaHome       = "$OpenSearchHome\jdk"

# Set Java environment to bundled JDK
$env:JAVA_HOME  = $JavaHome
$env:Path       = "$JavaHome\bin;$env:Path"

# Legacy alias kept for compatibility with rest of script
$AppRoot        = $ProjectRoot
...
if (-not $RedisService) {
    ...
    & $NssmPath install Redis-Datastore "$RedisPath" "`"$BinDir\Redis-x64-3.2.100\redis.windows-service.conf`""
    & $NssmPath set Redis-Datastore AppDirectory "$BinDir\Redis-x64-3.2.100"
...
if (-not (Test-Path ".venv\Scripts\python.exe")) {
    Write-Host "   Local E-drive virtual environment missing at $AppRoot\.venv\Scripts\python.exe" -ForegroundColor Red
    Write-Host "   Create it inside this repository on E-drive, then rerun start_services.ps1." -ForegroundColor Yellow
    throw "Missing local virtual environment"
}
else {
