...
$AppRoot        = "C:\Users\hp212560601\Music\DocumentSearch_v8\DocumentSearch"
$NssmPath       = "C:\Users\hp212560601\Tools\nssm\nssm-2.24-101-g897c7ad\win64\nssm.exe"
$RedisPath      = "C:\Users\hp212560601\Downloads\Redis-x64-3.2.100\redis-server.exe"
$OpenSearchBin  = "C:\opensearch-2.12.0\bin\opensearch.bat"
$OpenSearchHome = "C:\opensearch-2.12.0"
...
$LegacyConflicts = @("OpenSearch", "OpenSearch2", "opensearch-service-x64")
...
$Directories = @(
    "D:\DocumentSearch\opensearch\data",
    "D:\DocumentSearch\opensearch\logs",
    "D:\DocumentSearch\opensearch\temp"
)
...
icacls.exe "D:\DocumentSearch" /grant "SYSTEM:(OI)(CI)F" /t /q
icacls.exe "D:\DocumentSearch" /grant "Administrators:(OI)(CI)F" /t /q
...
if (-not $OSService) {
    Write-Host "   Service not found. Registering OpenSearch-Service..." -ForegroundColor Custom
    & $NssmPath install OpenSearch-Service "$OpenSearchBin"
    & $NssmPath set OpenSearch-Service AppDirectory "$OpenSearchHome\bin"
    & $NssmPath set OpenSearch-Service AppStdout "D:\DocumentSearch\opensearch\logs\nssm-stdout.log"
    & $NssmPath set OpenSearch-Service AppStderr "D:\DocumentSearch\opensearch\logs\nssm-stderr.log"
...
elseif ($OSService.Status -ne "Running") {
    ...
    Write-Host "   OpenSearch-Service is already running safely. Skipping layout modification." -ForegroundColor Gray
}
...
if (-not $RedisService) {
    ...
    & $NssmPath install Redis-Datastore "$RedisPath"
...
elseif ($RedisService.Status -ne "Running") {
    Write-Host "   Redis-Datastore is stopped. Starting server stack..." -ForegroundColor White
    Start-Service -Name "Redis-Datastore"
    Write-Host "   Redis-Datastore service is now online." -ForegroundColor Green
}
else {
    Write-Host "   Redis-Datastore is already active on Port 6379. Skipping." -ForegroundColor Gray
}
...
if (-not (Test-Path ".venv\Scripts\python.exe")) {
    Write-Host "   Virtual Environment corrupted or missing! Commencing deep layout rebuild..." -ForegroundColor Custom
