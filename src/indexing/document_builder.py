...
class DocumentBuilder:
    ...
    def _lookup_parent_info(self, parent_hash: str) -> Optional[Dict[str, str]]:
        """Look up parent file path and file name from Redis"""
        if not parent_hash:
            return None
        try:
            r = self._get_redis()
            if r:
                val = r.hget('docsearch:parent_info', parent_hash)
                if val:
    ...
    def build_document(self, document_json: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if parent_hash:
                ...
                parent_info = self._lookup_parent_info(parent_hash)
                if parent_info:
                    document['parent_path'] = parent_info.get('file_path', '')
                    document['parent_name'] = parent_info.get('file_name', '')
                else:
