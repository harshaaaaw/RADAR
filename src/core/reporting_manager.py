...
class _ReportingManager:
    ...
    def write_page_segmentation_breakdown(
        ...
        """Persist per-page visual zone breakdown metrics."""
        self._ensure_schema()
        def _insert(conn: sqlite3.Connection) -> None:
            conn.execute(
                """
    ...
    def get_page_segmentation_breakdown(self, smart_id: str) -> List[Dict[str, Any]]:
        """Get per-page segmentation breakdown for a specific document."""
        self._ensure_schema()
        def _read(conn: sqlite3.Connection) -> List[Dict[str, Any]]:
            rows = conn.execute(
                """
    ...
    def log_snippet_suppression(
        ...
        """Write a machine-audit record for an auto-suppressed snippet."""
        self._ensure_schema()
        def _insert(conn: sqlite3.Connection) -> None:
            conn.execute(
                """
                INSERT INTO snippet_suppression_log (
                    ...
                    snippet_type, accuracy_impact, suppressed_at, worker_id
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    smart_id,
    ...
    def add_opensearch_retry(
        ...
        """Queue a failed OpenSearch update for retry."""
        self._ensure_schema()
        def _insert(conn: sqlite3.Connection) -> None:
            conn.execute(
                """
    ...
    def create_snippet_review(
        ...
        """Create a pending snippet review record (idempotent)."""
        self._ensure_schema()
        def _insert(conn: sqlite3.Connection) -> None:
            conn.execute(
                """
    ...
    def update_snippet_review_status(
        ...
        """Update a snippet review status and recalculate document-level enhanced accuracy."""
        self._ensure_schema()
        def _update(conn: sqlite3.Connection) -> None:
                ...
                if baseline_row:
                    ...
                    if status in ("accepted", "rejected"):
                        conn.execute(
                            ...
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                            """,
                            (review_id, smart_id, status, status, reviewed_by, review_reason,
    ...
    def revert_snippet_review(
        ...
        """Revert a snippet review decision back to 'pending' and recalculate accuracy."""
        self._ensure_schema()
        def _revert(conn: sqlite3.Connection) -> None:
            ...
            if old_status == "pending":
                return  # Already pending — silent no-op
            ...
            # Recalculate accuracy after revert
            accepted_rows = conn.execute(
                "SELECT SUM(accuracy_impact) as total_accepted FROM snippet_reviews WHERE smart_id = ? AND status = 'accepted'",
                (smart_id,)
            ).fetchone()
            ...
            if baseline_row:
                ...
                conn.execute(
                    "UPDATE file_state SET enhanced_accuracy = ?, approval_status = ? WHERE file_key = ?",
                    (enhanced_acc, app_status, file_key)
                )
            else:
                enhanced_acc = 0.0
            ...
            try:
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (review_id, smart_id, action, "revert", "Dashboard User",
                     f"Reverted decision from {old_status}",
                     reviewed_at, accuracy_before, enhanced_acc, snippet_type, file_name)
                )
            except Exception:
                pass
    ...
    def get_docs_with_reviews(self) -> List[Dict[str, Any]]:
        ...
        ``full_page`` type snippets are counted separately and excluded from
        the headline counts so the label/metrics match the visible review items.
        """
        self._ensure_schema()
        def _read(conn: sqlite3.Connection) -> List[Dict[str, Any]]:
            # Ensure indexes exist for performance
            try:
                conn.execute("CREATE INDEX IF NOT EXISTS idx_sr_smart_id_type ON snippet_reviews(smart_id, snippet_type)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_sr_smart_id_status ON snippet_reviews(smart_id, status)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_fs_smart_id ON file_state(smart_id)")
            except Exception:
            ...
            rows = conn.execute(
                ...
                SELECT f.smart_id, f.file_name, f.file_path, f.extraction_accuracy,
                       ...
                       COUNT(CASE WHEN r.status = 'pending'  AND r.snippet_type != 'full_page' THEN 1 END) as pending_count,
                       COUNT(CASE WHEN r.status = 'accepted' AND r.snippet_type != 'full_page' THEN 1 END) as accepted_count,
                       COUNT(CASE WHEN r.status = 'rejected' AND r.snippet_type != 'full_page' THEN 1 END) as rejected_count,
                       COUNT(CASE WHEN r.snippet_type != 'full_page' THEN 1 END) as total_snippets,
                       SUM(CASE WHEN r.status = 'pending' AND r.snippet_type != 'full_page' THEN r.accuracy_impact ELSE 0 END) as pending_impact
                FROM snippet_reviews r
                JOIN file_state f ON r.smart_id = f.smart_id
                GROUP BY f.smart_id
                HAVING COUNT(CASE WHEN r.snippet_type != 'full_page' THEN 1 END) > 0
                ORDER BY COUNT(CASE WHEN r.status = 'pending' AND r.snippet_type != 'full_page' THEN 1 END) DESC, f.file_name
                """
            ).fetchall()
            return [dict(row) for row in rows]
        return self._execute_with_retry(_read, commit=False)
...
def revert_snippet_review(review_id: str) -> None:
    """Revert a snippet review decision back to pending."""
    _get_manager().revert_snippet_review(review_id=review_id)
...
def log_snippet_suppression(
    ...
    """Record an auto-suppressed snippet in the machine audit trail."""
...
def add_opensearch_retry(smart_id: str, review_id: str, payload: dict) -> None:
    _get_manager().add_opensearch_retry(smart_id=smart_id, review_id=review_id, payload=payload)
