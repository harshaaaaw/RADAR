...
import warnings
# Ignore Pydantic's protected namespace warnings
...
class _SuppressWebSocketClosed(logging.Filter):
    """Suppress harmless 'WebSocketClosedError' noise from Tornado/asyncio.

    This happens when the browser client disconnects while Streamlit's
    Tornado server still has a pending write queued; the error is benign
    and expected during normal page refreshes / tab closes.
    """
    def filter(self, record: logging.LogRecord) -> bool:
        msg = record.getMessage()
        return "WebSocketClosedError" not in msg and "StreamClosedError" not in msg

logging.getLogger("asyncio").addFilter(_SuppressWebSocketClosed())
...
# Streamlit can probe torch internals during file-watch scans and emit noisy
# non-fatal messages such as "Examining the path of torch.classes raised...".
# Keep the dashboard logs clean by silencing only that watcher logger.
...
REPO_ROOT = Path(__file__).resolve().parents[2]       # → E:\DocumentSearch
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from core.config_manager import get_config  # noqa: E402
from core.logging_manager import setup_logging  # noqa: E402
from core.queue_manager import get_queue_manager, reset_queue_manager  # noqa: E402

# Initialize structured logging so dashboard logs go to runtime/logs/
try:
    setup_logging()
except Exception:
    pass
...
def _get_parent_doc_count(_os_client_id: int) -> int:
    """Return the count of parent (non-embedded) documents in OpenSearch.

    Uses _os_client_id as the cache key so the result is refreshed
    whenever the client instance changes.  Returns -1 when unavailable
    (caller should fall back to the Redis-based counter).
    """
    ...
    try:
        if not os_client.client.indices.exists(index=os_client.index_name):
            return 0
        # Use must_not to exclude embedded docs — handles both explicit
        # is_embedded:true AND documents where the field is missing.
        resp = os_client.client.count(
            index=os_client.index_name,
            body={"query": {"bool": {"must_not": [
                {"term": {"is_embedded": True}},
                {"wildcard": {"file_path": "*data?embedded*"}},
            ]}}}
        )
...
@st.cache_data(ttl=30, show_spinner=False)
def _get_embedded_discovered_count() -> int:
    """Return how many embedded child files were discovered.

    Uses Redis `docsearch:parent_map` cardinality, where each child hash maps
    to its parent hash.
    """
    try:
        qm = get_queue_manager()
        r = getattr(qm, "client", None)
        if r is None:
            return -1
        return int(r.hlen("docsearch:parent_map") or 0)
    except Exception:
        return -1


@st.cache_data(ttl=30, show_spinner=False)
def _get_parent_failed_count() -> int:
    """Return failed-file count excluding embedded children."""
    try:
        qm = get_queue_manager()
        r = getattr(qm, "client", None)
        if r is None:
        ...
        cursor = 0
        while True:
            cursor, data = r.hscan("docsearch:failed", cursor, count=250)
            for _, raw in (data or {}).items():
                try:
                    info = json.loads(raw)
                except Exception:
                    continue

                file_path = str(info.get("file_path", "") or "").replace("\\", "/").lower()
                if "/data/embedded/" in file_path:
                    continue
                parent_failed += 1
...
def _get_parent_stage_counts() -> Dict[str, Dict[str, int]]:
    """Return parent-only pending/processing/completed counts per stage.

    Embedded children (typically under /data/embedded/) are excluded.
    """
    empty = {
        "extraction": {"pending": 0, "processing": 0, "completed": 0},
    ...
    try:
        ...
        def _is_embedded_path(path: str) -> bool:
            p = str(path or "").replace("\\", "/").lower()
            return "/data/embedded/" in p
        ...
        # Build parent file-id set from path->id map.
        parent_ids: Set[str] = set()
        cursor = 0
        while True:
            cursor, data = r.hscan(qm.HASH_FILE_PATHS, cursor, count=500)
            for file_path, file_id in (data or {}).items():
                if not _is_embedded_path(file_path):
                    parent_ids.add(str(file_id))
            if cursor == 0:
                break
        ...
        def _count_pending_zset(key: str) -> int:
            count = 0
            for raw in r.zrange(key, 0, -1):
                item = _parse_item(raw)
                fid = str(item.get("file_id") or item.get("id") or "")
                fpath = str(item.get("file_path") or "")
                if fid and fid in parent_ids:
                    count += 1
                elif fpath and not _is_embedded_path(fpath):
                    count += 1
            return count

        def _count_pending_list(key: str) -> int:
            count = 0
        ...
        def _count_processing(prefix: str) -> int:
            count = 0
            keys = []
            try:
                keys = qm._get_worker_keys(prefix)  # Reuses queue-manager key cache.
            except Exception:
        ...
        for cat in ["tiny", "small", "medium", "large"]:
            ext_pending += _count_pending_zset(f"{qm.QUEUE_EXTRACTION}:{cat}")
        idx_pending = _count_pending_list(qm.QUEUE_INDEXING)
        ocr_pending = _count_pending_zset(qm.QUEUE_OCR)
        tag_pending = _count_pending_list(qm.QUEUE_TAGGING)

        # Processing counts
        ext_processing = _count_processing(qm.PROCESSING_EXTRACTION)
        idx_processing = _count_processing(qm.PROCESSING_INDEXING)
        ocr_processing = _count_processing(qm.PROCESSING_OCR)
        tag_processing = _count_processing(qm.PROCESSING_TAGGING)

        # Completed counts (derived from parent file metadata)
        ext_completed = 0
        idx_completed = 0
        ocr_completed = 0
        tag_completed = 0
        ...
        for fid in parent_ids:
            ...
            if len(batch) >= 500:
                ...
                for bid in batch:
                    pipe.hmget(f"{qm.HASH_FILES}:{bid}", "extraction_completed_at", "indexed_at", "ocr_completed", "tagged_at")
                rows = pipe.execute()
                for row in rows:
                    if row and row[0]:
                        ext_completed += 1
        ...
        return {
            "extraction": {"pending": ext_pending, "processing": ext_processing, "completed": ext_completed},
            "indexing": {"pending": idx_pending, "processing": idx_processing, "completed": idx_completed},
            "ocr": {"pending": ocr_pending, "processing": ocr_processing, "completed": ocr_completed},
            "tagging": {"pending": tag_pending, "processing": tag_processing, "completed": tag_completed},
        }
    except Exception:
        return empty
...
def extract_summary(queue_stats: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Indexing metrics — total should reflect all files that completed
        # extraction (i.e. entered the indexing pipeline), not just current queue state.
        ...
        if indexing_total == 0:
            # Use extraction_completed as the true denominator for indexing progress
            ext_completed = safe_int(extraction_total_stats.get("completed"))
            if ext_completed > 0:
                indexing_total = ext_completed
            else:
...
def render_dashboard() -> None:
    # Trigger check for running test.py logic
    trigger_path = REPO_ROOT / "run_trigger.txt"
    if trigger_path.exists():
        ...
        try:
            ...
            if str(PROJECT_ROOT) not in sys.path:
                sys.path.insert(0, str(PROJECT_ROOT))
            import test
            _test_pdf = REPO_ROOT / "test_data" / "real_funsd_form_012.pdf"
            res = test.extract_text(str(_test_pdf))
            (REPO_ROOT / "ocr_output.txt").write_text(res, encoding="utf-8")
            (REPO_ROOT / "run_status.txt").write_text("SUCCESS", encoding="utf-8")
        except Exception as e:
            import traceback
            err_msg = f"Error: {e}\n{traceback.format_exc()}"
            (REPO_ROOT / "ocr_output.txt").write_text(err_msg, encoding="utf-8")
            (REPO_ROOT / "run_status.txt").write_text("FAILED", encoding="utf-8")
...
def _render_snippet_review_safe(config: Any) -> None:
    """Load and render the Snippet Review tab with a loading state.

    When switching from Search/Live Audit, the heavy imports (VisualMemoryEngine,
    PIL, etc.) can stall the first render, causing a blank page.  Wrapping in
    st.spinner and catching import errors prevents the blank-page issue.
    """
    try:
        from ui.review_tab import render_snippet_review_tab
    except Exception as import_err:
        st.error(f"Failed to load Snippet Review module: {import_err}")
        return
...
def render_sidebar(config: Any, os_client: Optional[OpenSearchClient], queue_manager: Any) -> None:
    ...
    with st.sidebar:
        ...
        try:
            ...
            raw_discovered = safe_get(discovered, 'files', default=0)
            ...
            # Prefer OpenSearch parent-only count so embedded files are not
            # inflating the "Searchable" figure shown in the sidebar.
            _sidebar_os = _get_os_client()
            _parent_count = _get_parent_doc_count(id(_sidebar_os)) if _sidebar_os else -1
            if _parent_count >= 0:
                total_searchable_root = _parent_count
            else:
                # Fallback: subtract known embedded count from Redis parent_map
                _embed_count = _get_embedded_discovered_count()
                if _embed_count > 0 and total_searchable_root > _embed_count:
                    total_searchable_root = total_searchable_root - _embed_count

            # Subtract embedded children from discovered count so sidebar
            # only shows parent files (aligned with System Monitor behaviour).
            embedded_discovered = _get_embedded_discovered_count()
            if embedded_discovered > 0:
                total_discovered = max(0, raw_discovered - embedded_discovered)
            else:
                total_discovered = raw_discovered

            # Also exclude embedded from failed count (parent-only)
            raw_failed = safe_get(failed, 'files', default=0)
            parent_failed = _get_parent_failed_count()
            total_failed = parent_failed if parent_failed >= 0 else raw_failed
            ...
            if show_as_files:
                ...
                st.markdown(f"**{total_searchable_root:,}** files")
                ...
                st.caption(f"{total_searchable_root:,} files")
            ...
            # Pipeline status (compact) \u2014 all counts are parent-only
            st.markdown("### \u2699\uFE0F Pipeline")

            # Subtract embedded children so pipeline progress shows parent files only
            _embedded_adj = _get_embedded_discovered_count()
            _embedded_adj = _embedded_adj if _embedded_adj > 0 else 0

            # Discovery
            discovery_total = max(0, (summary.get('discovered_total', 0) or 0) - _embedded_adj)
            discovery_completed = min(summary.get('discovery_completed', 0) or 0, discovery_total)
            discovery_pending = max(0, (summary.get('discovery_pending', 0) or 0))
            ...
            # Extraction
            extraction_total = max(0, (summary.get('extraction_total', 0) or 0) - _embedded_adj)
            extraction_completed = min(summary.get('extraction_completed', 0) or 0, extraction_total)
            ...
            # Indexing
            indexing_total = max(0, (summary.get('indexing_total', 0) or 0) - _embedded_adj)
            indexing_completed = min(summary.get('indexing_completed', 0) or 0, indexing_total)
            ...
            with st.sidebar.expander("🛠️ System Diagnostics", expanded=False):
                try:
                    r = getattr(queue_manager, 'client', None)
                    if r:
                        st.markdown("**Raw Redis Counters**")
                        st.write(f"• `discovered`: `{r.get('docsearch:counter:discovered')}`")
                        st.write(f"• `completed`: `{r.get('docsearch:counter:completed')}`")
                        st.write(f"• `ocr_completed`: `{r.get('docsearch:counter:ocr_completed')}`")
                        st.write(f"• `ocr_queue` size: `{r.zcard('docsearch:queue:ocr')}`")
                        st.write(f"• `failed` size: `{r.hlen('docsearch:failed')}`")
                        st.write(f"• `parent_info` size: `{r.hlen('docsearch:parent_info')}`")
                        st.write(f"• `parent_map` size: `{r.hlen('docsearch:parent_map')}`")
                except Exception as diag_err:
                    st.write(f"Diag error: {diag_err}")
...
def render_search_tab(config: Any, os_client: Optional[OpenSearchClient]) -> None:
    ...
    if (query and len(query) >= 2) or search_button:
        ...
        # Show recent documents with same advanced filters applied
        render_recent_documents(os_client, filters=filters)
...
def extract_snippet_manually(text: str, query: str, max_length: int = 180) -> str:
    ...
    for term in query_terms:
        if not term:
            continue
        # Strip internal commas/periods for digits (OCR confuses , and .)
        normalized_term = re.sub(r'(?<=\d)[.,]+(?=\d)', '', term)
        if normalized_term.isdigit():
            # Build pattern that matches this number with any combination of . and , separators
            pattern = re.compile(r'(?<!\d)' + r'[.,\s]?'.join(list(normalized_term)) + r'(?!\d)', re.IGNORECASE)
        else:
            # For non-numeric terms containing digits with separators, also try swapped separators
            if re.search(r'\d[.,]\d', term):
                dot_variant = term.replace('.', ',')
                comma_variant = term.replace(',', '.')
                escaped_variants = '|'.join(re.escape(v) for v in {term, dot_variant, comma_variant} if v)
                pattern = re.compile(f'(?:{escaped_variants})', re.IGNORECASE)
            else:
...
def _generate_ocr_variants(query: str) -> List[str]:
    ...
    - cl <-> d
    - vv <-> w
    - , <-> . (comma/period confusion in numbers due to unclear images)
    """
    if not query:
        return [query]
    ...
    # Comma/period interchange for numeric contexts (OCR confuses , and .)
    if ',' in query_lower:
        variants.add(query_lower.replace(',', '.'))
    if '.' in query_lower:
        variants.add(query_lower.replace('.', ','))
...
def perform_search(os_client: OpenSearchClient, query: str, limit: int = 20, filters: Optional[Dict[str, List[str]]] = None) -> List[Dict[str, Any]]:
    ...
    highlight_block = {
        "fields": {
            ...
            "metadata.title": {"number_of_fragments": 0},
            "metadata.author": {"number_of_fragments": 0},
            "metadata.subject": {"fragment_size": 200, "number_of_fragments": 2},
            "metadata.keywords": {"fragment_size": 200, "number_of_fragments": 2},
            "metadata.creator": {"number_of_fragments": 0},
            "metadata.producer": {"number_of_fragments": 0},
        },
        "pre_tags": ["**"],
        "post_tags": ["**"],
    ...
    if slash_command:
                    ...
                    "query": {
                        "dis_max": {
                            "queries": [
                                ...
                                {"match_phrase": {"metadata.title": {"query": normalized_query, "boost": 70, "slop": 0}}},
                                {"match_phrase": {"metadata.author": {"query": normalized_query, "boost": 60, "slop": 0}}},
                                {"match_phrase": {"metadata.subject": {"query": normalized_query, "boost": 50, "slop": 0}}},
                                {"match_phrase": {"metadata.keywords": {"query": normalized_query, "boost": 50, "slop": 0}}},
                                {"match_phrase": {"metadata.creator": {"query": normalized_query, "boost": 40, "slop": 0}}},
                            ],
                            "tie_breaker": 0.1
                        }
                    },
                    "functions": [
                        {"filter": {"term": {"is_embedded": True}}, "weight": 0.3}
                    ],
                    "boost_mode": "multiply"
                }
            },
        ...
        content_fields = [
            ...
            "metadata.title^12",       # Document title is highly relevant
            "metadata.author^10",      # Author name match is relevant
            "metadata.subject^8",      # Subject/description
            "metadata.keywords^8",     # Document keywords
            "metadata.creator^5",      # Creator application/tool
        ]
        ...
        # --- Numeric dot/comma normalization: search all content fields ---
        # When a query contains digits with . or , separators, add the swapped
        # variant at high priority across all content fields so "2.480.821"
        # finds "2,480,821" and "2480821.04" finds "2,480,821.04".
        _has_numeric_sep = bool(re.search(r'\d[.,]\d', normalized_query))
        if _has_numeric_sep:
            if '.' in normalized_query:
                _dot_variant = normalized_query.replace('.', ',')
                bool_clauses["should"].append({"match_phrase": {"main_content": {"query": _dot_variant, "boost": 45, "slop": 0}}})
                bool_clauses["should"].append({"match_phrase": {"ocr_content": {"query": _dot_variant, "boost": 45, "slop": 1}}})
                bool_clauses["should"].append({"match_phrase": {"embedded_content": {"query": _dot_variant, "boost": 35, "slop": 0}}})
                bool_clauses["should"].append({"match_phrase": {"reviewed_content": {"query": _dot_variant, "boost": 45, "slop": 1}}})
            if ',' in normalized_query:
                _comma_variant = normalized_query.replace(',', '.')
                bool_clauses["should"].append({"match_phrase": {"main_content": {"query": _comma_variant, "boost": 45, "slop": 0}}})
                bool_clauses["should"].append({"match_phrase": {"ocr_content": {"query": _comma_variant, "boost": 45, "slop": 1}}})
                bool_clauses["should"].append({"match_phrase": {"embedded_content": {"query": _comma_variant, "boost": 35, "slop": 0}}})
                bool_clauses["should"].append({"match_phrase": {"reviewed_content": {"query": _comma_variant, "boost": 45, "slop": 1}}})
            # Also search for the stripped version (no separators) for partial matches
            _stripped = re.sub(r'(?<=\d)[.,](?=\d)', '', normalized_query)
            if _stripped != normalized_query:
                bool_clauses["should"].append({"match_phrase": {"main_content": {"query": _stripped, "boost": 30, "slop": 0}}})
                bool_clauses["should"].append({"match_phrase": {"ocr_content": {"query": _stripped, "boost": 30, "slop": 1}}})
                    ...
                    "must": {
                        "function_score": {
                            ...
                            "functions": [
                                ...
                                # Demote embedded files so parents always rank first
                                {
                                    "filter": {"term": {"is_embedded": True}},
                                    "weight": 0.3
                                }
                            ],
                            "score_mode": "sum",
    ...
    for hit in response['hits']['hits']:
        ...
        result = {
            ...
            'extraction_time_ms': source.get('extraction_time_ms', 0),
            # mime_type lives inside metadata in OpenSearch; fall back to top-level for
            # older documents that stored it directly.
            'mime_type': (source.get('metadata') or {}).get('mime_type') or source.get('mime_type', ''),
...
def get_downloadable_text(result: Dict[str, Any]) -> str:
    """Combine extracted text from document fields into a clean downloadable format."""
    parts = []

    metadata = result.get('metadata') or {}
    mime = result.get('mime_type') or metadata.get('mime_type', '')
    ...
    parts.append(f"Path        : {result.get('filepath', '')}")
    if result.get('file_type'):
        parts.append(f"Type        : {result['file_type'].upper()}")
    if mime:
        parts.append(f"MIME        : {mime}")
    if page_count:
        parts.append(f"Pages       : {page_count}")
    if conf_str:
        parts.append(f"OCR Conf.   : {conf_str}")
    if result.get('indexed_at'):
        parts.append(f"Indexed At  : {result['indexed_at']}")
    parts.append("")
    ...
    main_content = str(result.get('main_content', '') or '').strip()

    # For PDFs the main_content field often stores just the filename slug (e.g.
    # "real_funsd_form_048") rather than actual prose.  Only show it as a
    # separate section when it appears to be real document text (> 80 chars and
    # differs from the filename stem).
    filename_stem = Path(result.get('filename', '')).stem.lower()
    main_is_trivial = (
        len(main_content) <= 80
        or main_content.lower().replace('_', '').replace('-', '').replace(' ', '') == filename_stem.replace('_', '').replace('-', '')
    )
    ...
    if main_content and not main_is_trivial:
...
def render_search_results(results: List[Dict[str, Any]], query: str) -> None:
    ...
    for i, result in enumerate(results):
        with st.container():
            ...
            # Parent-child lineage (embedded docs) — show parent folder path above embedded file
            if result.get('is_embedded'):
                parent_path = result.get("parent_path") or ""
                parent_name = result.get("parent_name") or ""
                parent_folder = str(Path(parent_path).parent) if parent_path else ""
                st.markdown(
                    f'<div style="color:#4B5563;font-size:0.82rem;margin-top:0.15rem;margin-bottom:0.1rem;">'
                    f'📂 <span style="color:#6B7280;">Parent Folder:</span> '
                    f'<span style="color:#374151;font-weight:500;">{_esc(parent_folder)}</span></div>'
                    f'<div style="color:#4B5563;font-size:0.88rem;font-weight:500;margin-bottom:0.4rem;">'
                    f'📁 Parent File: <span style="color:#1E40AF;font-weight:600;">{_esc(parent_name)}</span>'
                    f' <span style="color:#6B7280;font-size:0.78rem;">({_esc(parent_path)})</span></div>',
                    unsafe_allow_html=True
                )
            ...
            btn_open, btn_text, btn_pdf = st.columns(3)
            ...
            with btn_open:
                if embedded_pdf is not None:
                    ...
                    st.markdown(
                        ...
                        f'style="display:block;text-align:center;padding:0.45rem 0.6rem;'
                    ...
                    if file_opened:
                        ...
                        st.error(file_open_error)

            with btn_text:
                st.download_button(
                    label="📥 Download Text",
                    data=get_downloadable_text(result),
            ...
            with btn_pdf:
                try:
                    from ui.pdf_report import generate_document_report
                    st.download_button(
                        label="📄 PDF Report",
                        data=generate_document_report(result),
...
def render_recent_documents(os_client: OpenSearchClient, limit: int = 15,
                            filters: Optional[Dict[str, List[str]]] = None) -> None:
    """Render recently indexed documents with optional filters."""
    try:
        # Check if index exists first
        if not os_client.client.indices.exists(index=os_client.index_name):
        ...
        # Build filter clauses from the advanced filters panel
        ...
        query = {
            ...
            "_source": [
                ...
                "mime_type", "indexed_at", "metadata", "smart_id",
                "main_content", "embedded_content", "ocr_content", "reviewed_content",
                "ocr_confidence", "ocr_completed", "needs_ocr", "extraction_time_ms",
                "parent_name", "parent_path", "parent_file", "is_embedded",
                "category", "department", "purpose",
            ]
        }
        ...
        for hit in response['hits']['hits']:
            ...
            docs.append({
                ...
                'extraction_time_ms': source.get('extraction_time_ms'),
                'is_embedded': source.get('is_embedded', False),
                'parent_file': source.get('parent_file', ''),
                'parent_path': source.get('parent_path', ''),
                'parent_name': source.get('parent_name', ''),
        ...
        _filter_note = f" (filtered)" if filters else ""
        st.markdown(f"### Recently Processed Documents ({len(docs)}{_filter_note})")

        for i, doc in enumerate(docs):
            with st.container():
                st.markdown('<div class="doc-card">', unsafe_allow_html=True)

                info_col, _ = st.columns([1, 0.01])
                with info_col:
                    ...
                    if doc.get('is_embedded'):
                        _p_path = doc.get("parent_path") or ""
                        _p_folder = str(Path(_p_path).parent) if _p_path else ""
                        st.markdown(
                            f'<div style="color:#4B5563;font-size:0.82rem;margin-top:0.15rem;margin-bottom:0.1rem;">'
                            f'📂 <span style="color:#6B7280;">Parent Folder:</span> '
                            f'<span style="color:#374151;font-weight:500;">{_esc(_p_folder)}</span></div>'
                            f'<div style="color:#4B5563;font-size:0.82rem;font-weight:500;margin-bottom:0.3rem;">'
                            f'📁 Parent File: <span style="color:#1E40AF;font-weight:600;">{_esc(doc.get("parent_name"))}</span>'
                            f' <span style="color:#6B7280;font-size:0.75rem;">({_esc(_p_path)})</span></div>',
                            unsafe_allow_html=True
                        )
                ...
                with btn_pdf_r:
                    try:
                        ...
                        st.download_button(
                            label="📄 PDF Report",
                            data=generate_document_report(doc),
...
def render_monitoring_tab(config: Any, queue_manager: Any) -> None:
    ...
    raw_discovered = summary["discovered_total"]
    raw_completed = summary["completed_total"]
    raw_failures = summary["total_failures"]
    total_duplicates = summary["duplicates"]

    # Parent-only normalization for System Monitor headline counts.
    # completed: authoritative from OpenSearch (is_embedded=false)
    # discovered: total discovered minus known embedded discovered children
    # failed: scan failed hash and exclude /data/embedded/ entries
    _mon_os = _get_os_client()
    parent_completed = _get_parent_doc_count(id(_mon_os)) if _mon_os else -1
    if parent_completed < 0:
        parent_completed = max(0, raw_completed)

    embedded_discovered = _get_embedded_discovered_count()
    if embedded_discovered >= 0:
        parent_discovered = max(0, raw_discovered - embedded_discovered)
    else:
        parent_discovered = max(parent_completed, raw_discovered)

    parent_failures = _get_parent_failed_count()
    if parent_failures < 0:
        parent_failures = max(0, raw_failures)

    total_discovered = parent_discovered
    total_completed = parent_completed
    total_failures = parent_failures

    # Overall progress (parent files only)
    ...
    # Parent-only per-stage stats for all monitor counters.
    # Fall back to raw queue stats if parent-only counts are empty (avoids showing all zeros).
    parent_stage = _get_parent_stage_counts()
    _parent_has_data = any(
        v.get("pending", 0) + v.get("processing", 0) + v.get("completed", 0) > 0
        for v in parent_stage.values()
    )
    if _parent_has_data:
        ext_stats = parent_stage.get("extraction", {}) or {}
        idx_stats = parent_stage.get("indexing", {}) or {}
        ocr_stats = parent_stage.get("ocr", {}) or {}
        tag_stats = parent_stage.get("tagging", {}) or {}
    else:
        ext_stats = {
            "pending": summary.get("extraction_pending", 0),
            "processing": summary.get("extraction_processing", 0),
            "completed": summary.get("extraction_completed", 0),
        }
        idx_stats = {
            "pending": summary.get("indexing_pending", 0),
            "processing": summary.get("indexing_processing", 0),
            "completed": summary.get("indexing_completed", 0),
        }
        ocr_stats = {
            "pending": summary.get("ocr_pending", 0),
            "processing": summary.get("ocr_processing", 0),
            "completed": summary.get("ocr_completed", 0),
        }
        tag_stats = {
            "pending": summary.get("tagging_pending", 0),
            "processing": summary.get("tagging_processing", 0),
            "completed": summary.get("tagging_completed", 0),
        }

    # Ensure indexing "completed" uses the authoritative counter when the
    # parent-stage scan didn't find completion timestamps but files are indexed.
    # COUNTER_ROOT_COMPLETED (via summary) is the most reliable source.
    idx_completed_from_summary = summary.get("indexing_completed", 0) or 0
    if int(idx_stats.get("completed", 0) or 0) < idx_completed_from_summary:
        idx_stats["completed"] = idx_completed_from_summary

    # Processing counts from the parent-stage scan can under-report when the
    # parent_ids set doesn't cover an item currently being processed. Use the
    # raw hlen-based count (from queue_stats) as a floor for processing metrics.
    for _stage, _stats_ref in [("ocr", ocr_stats), ("extraction", ext_stats),
                                ("indexing", idx_stats), ("tagging", tag_stats)]:
        raw_processing = summary.get(f"{_stage}_processing", 0) or 0
        if int(_stats_ref.get("processing", 0) or 0) < raw_processing:
            _stats_ref["processing"] = raw_processing

    total_in_flight = max(
        0,
        int(ext_stats.get("pending", 0) or 0)
        + int(ext_stats.get("processing", 0) or 0)
        + int(idx_stats.get("pending", 0) or 0)
        + int(idx_stats.get("processing", 0) or 0)
        + int(ocr_stats.get("pending", 0) or 0)
        + int(ocr_stats.get("processing", 0) or 0)
        + int(tag_stats.get("pending", 0) or 0)
        + int(tag_stats.get("processing", 0) or 0)
    )

    # Success rate (parent files only)
    ...
    if show_as_files:
        with col1:
            st.metric(
                ...
                help="Parent files discovered (embedded files excluded)"
            )
        ...
        with col2:
            st.metric(
                ...
                help="Parent files fully processed (embedded files excluded from this count)"
            )
        ...
        with col3:
            st.metric(
                ...
                help="Parent files currently in pipeline (embedded files excluded)"
            )
        ...
        with col4:
            st.metric(
                ...
                delta_color="normal",
                help="Parent files that encountered errors during processing"
            )
        ...
        if total_to_process > 0:
            st.progress(
                ...
                text=f"Overall Progress: {overall_progress:.1f}% ({total_completed:,} / {total_to_process:,} parent files)"
            )
    else:
    ...
    with col1:
        ...
        st.metric("Pending", format_number(ext_stats.get("pending", 0)))
        st.metric("Processing", format_number(ext_stats.get("processing", 0)))
        st.metric("Done", format_number(ext_stats.get("completed", 0)))
    
    with col2:
        st.markdown("**\U0001F5C2\uFE0F Indexing**")
        st.metric("Pending", format_number(idx_stats.get("pending", 0)))
        st.metric("Processing", format_number(idx_stats.get("processing", 0)))
        st.metric("Done", format_number(idx_stats.get("completed", 0)))
    
    with col3:
        st.markdown("**\U0001F50D OCR Queue**")
        st.metric("Pending", format_number(ocr_stats.get("pending", 0)))
        st.metric("Processing", format_number(ocr_stats.get("processing", 0)))
        st.metric("Done", format_number(ocr_stats.get("completed", 0)))
    ...
    try:
        ...
        rem_extract = int(ext_stats.get('pending', 0) or 0) + int(ext_stats.get('processing', 0) or 0)
        rem_index = int(idx_stats.get('pending', 0) or 0) + int(idx_stats.get('processing', 0) or 0)
        rem_ocr = int(ocr_stats.get('pending', 0) or 0) + int(ocr_stats.get('processing', 0) or 0)
    ...
    try:
        comp_files = total_completed  # parent-only count (from OpenSearch)
        ...
        if failures:
            ...
            try:
                ...
                failed_files = [
                    ...
                    if '/data/embedded/' not in (f.get('file_path', '') or '').replace('\\', '/')
                ]
                if failed_files:
    ...
    # =========================================================================
    # DATA EXPORT & SYSTEM REPORT — side by side
    # =========================================================================
    st.markdown("---")
    col_export, col_report = st.columns(2)

    # --- Column 1: Data Export (Parquet) ---
    with col_export:
        st.subheader("📦 Data Export")
        st.caption(
            "Download all processed document data as a Parquet file for "
            "ingestion into data fabric / analytics platforms."
        )
        st.markdown(
            """
            <div style="font-size:0.78rem; color:#64748B; line-height:1.5; margin-bottom:0.8rem;">
                <b>Includes:</b> File metadata, Smart ID, category, department, purpose,
                key names, locations, dates, OCR confidence, extracted content,
                reviewed labels, tags, and parent-child relationships.<br>
                <b>Format:</b> Apache Parquet — columnar, compressed, compatible with
                Spark, Databricks, Snowflake, BigQuery, pandas, and all major data platforms.
            </div>
            """,
            unsafe_allow_html=True,
        )
        if st.button("📦 Generate Parquet Export", use_container_width=True, key="gen_parquet_export"):
            try:
                ...
                if parquet_bytes:
                    st.session_state["_parquet_export_bytes"] = parquet_bytes
                    st.success("Export generated — click Download below.")
                else:
                    st.warning("No processed documents found to export.")
            except Exception as _pq_err:
                st.error(f"Parquet export failed: {_pq_err}")

        if st.session_state.get("_parquet_export_bytes"):
            _ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            st.download_button(
                label="⬇️ Download Parquet File",
                data=st.session_state["_parquet_export_bytes"],
    ...
    with col_report:
        ...
        if st.button("🖨 Generate System Report", use_container_width=True, key="gen_sys_report"):
            try:
                ...
                _s_stats = get_cached_size_stats()
                _pdf_bytes = generate_system_report(_q_stats, _s_stats)
                st.session_state["_sys_report_bytes"] = _pdf_bytes
                st.success("Report generated — click Download below.")
            except Exception as _rpt_err:
                st.error(f"Report generation failed: {_rpt_err}")

        if st.session_state.get("_sys_report_bytes"):
            _ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            st.download_button(
                label="⬇️ Download System Report PDF",
                data=st.session_state["_sys_report_bytes"],
...
def _generate_parquet_export() -> Optional[bytes]:
    """Export all processed documents from OpenSearch as a Parquet file.

    Scrolls through the entire index, flattens each document into a flat row,
    and returns the Parquet binary (snappy-compressed).
    """
    import io
    ...
    # Fields to retrieve from OpenSearch
    source_fields = [
        "smart_id", "file_name", "file_path", "file_size", "file_type",
        "file_hash", "mime_type", "indexed_at", "extraction_time_ms",
        "is_embedded", "parent_file", "parent_path", "parent_name",
        "category", "department", "purpose", "confidentiality",
        "key_names", "amount_found", "important_dates", "location_mentioned",
        "dynamic_subtags", "tagging_status", "tag_confidence",
        "tagger_version", "taxonomy_version",
        "needs_ocr", "ocr_completed", "ocr_confidence",
        "main_content", "embedded_content", "ocr_content", "reviewed_content",
        "metadata", "is_duplicate", "duplicate_of",
    ]
    ...
    try:
        ...
        while hits:
            for hit in hits:
                src = hit["_source"]
                rows.append(_flatten_os_doc(src))

            resp = os_client.client.scroll(scroll_id=scroll_id, scroll=scroll_timeout)
            scroll_id = resp.get("_scroll_id")
            hits = resp["hits"]["hits"]
        ...
        if scroll_id:
            try:
                os_client.client.clear_scroll(scroll_id=scroll_id)
            except Exception:
                pass
    except Exception as e:
        raise RuntimeError(f"OpenSearch scroll failed: {e}") from e
    ...
    # Truncate large text columns to keep parquet manageable
    _text_cols = ["main_content", "embedded_content", "ocr_content"]
    _max_text_len = 50000
    for col in _text_cols:
        if col in df.columns:
            df[col] = df[col].apply(
                lambda v: v[:_max_text_len] if isinstance(v, str) and len(v) > _max_text_len else v
            )
...
def _flatten_os_doc(src: Dict[str, Any]) -> Dict[str, Any]:
    """Flatten an OpenSearch document source into a flat dict for parquet export."""
    ...
    return {
        "smart_id": src.get("smart_id", ""),
        "file_name": src.get("file_name", ""),
        "file_path": src.get("file_path", ""),
        "file_size_bytes": src.get("file_size", 0),
        "file_type": src.get("file_type", ""),
        "file_hash": src.get("file_hash", ""),
        "mime_type": src.get("mime_type", ""),
        "indexed_at": src.get("indexed_at", ""),
        "extraction_time_ms": src.get("extraction_time_ms", 0),
        # Classification / tagging
        "category": src.get("category", ""),
        "department": src.get("department", ""),
        "purpose": src.get("purpose", ""),
        "confidentiality": src.get("confidentiality", ""),
        "key_names": _list_to_str(src.get("key_names")),
        "amount_found": _list_to_str(src.get("amount_found")),
        "important_dates": _list_to_str(src.get("important_dates")),
        "location_mentioned": _list_to_str(src.get("location_mentioned")),
        "dynamic_subtags": _list_to_str(src.get("dynamic_subtags")),
        "tagging_status": src.get("tagging_status", ""),
        "tag_confidence": src.get("tag_confidence"),
        "tagger_version": src.get("tagger_version", ""),
        "taxonomy_version": src.get("taxonomy_version", ""),
        # OCR
        "needs_ocr": src.get("needs_ocr", False),
        "ocr_completed": src.get("ocr_completed", False),
        "ocr_confidence": src.get("ocr_confidence"),
        # Content (may be truncated)
        "main_content": src.get("main_content", ""),
        "embedded_content": src.get("embedded_content", ""),
        "ocr_content": src.get("ocr_content", ""),
        "reviewed_content": src.get("reviewed_content", ""),
        # Parent-child
        "is_embedded": src.get("is_embedded", False),
        "parent_file": src.get("parent_file", ""),
        "parent_path": src.get("parent_path", ""),
        "parent_name": src.get("parent_name", ""),
        "is_duplicate": src.get("is_duplicate", False),
        "duplicate_of": src.get("duplicate_of", ""),
        # Metadata flattened
        ...
        "meta_created_date": meta.get("created", ""),
        "meta_modified_date": meta.get("modified", ""),
    }
