...
def _resolve_snippet_path(snippet_path: Any, working_root: Any) -> Any:
    """Resolve legacy snippet paths into current workspace data path."""
    if snippet_path is None:
        return None
    is_str = isinstance(snippet_path, str)
    sp = Path(snippet_path)
    wr = Path(working_root) if working_root else Path()
    
    resolved = sp
    if sp.exists():
        resolved = sp
    else:
        normalized = str(sp).replace("\\", "/")
        marker = "/data/review_snippets/"
        if marker in normalized:
            relative_part = normalized.split(marker, 1)[1]
            mapped = wr.parent / "data" / "review_snippets" / Path(relative_part)
            if mapped.exists():
                resolved = mapped

    return str(resolved) if is_str else resolved
...
def _build_document_page_link(file_path: str, page_num: int) -> Optional[str]:
    ...
    try:
        ...
        if not resolved:
            return None
        p = Path(resolved)
        resolved_uri = p.resolve().as_uri()
        if page_num > 1:
            resolved_uri += f"#page={page_num}"
        return resolved_uri
    except Exception:
        return None
...
@st.cache_data(show_spinner=False)
def _cached_is_printed_font(snippet_path_str: str) -> bool:
    try:
        arr = np.array(Image.open(snippet_path_str).convert("L"))
        if arr.ndim != 2:
            return False
...
def _is_printed_font_review_snippet(snippet: Dict[str, Any], working_root: Path) -> bool:
    ...
    if not snippet_path.exists():
        return False
    return _cached_is_printed_font(str(snippet_path))
...
@st.cache_data(show_spinner=False)
def _cached_is_text_like(snippet_path_str: str, snippet_type: str) -> bool:
    tesseract = _get_review_tesseract()
    if not tesseract:
        return False
    try:
        ocr_result = tesseract.extract_text(snippet_path_str)
        if not ocr_result:
            return False
...
def _is_text_like_review_snippet(snippet: Dict[str, Any], working_root: Path) -> bool:
    ...
    if not snippet_path.exists():
        return False
    return _cached_is_text_like(str(snippet_path), str(snippet.get("snippet_type") or ""))
...
def _render_snippet_card(
    ...
    with st.container(border=True):
        ...
        with st.expander("Tesseract Extracted", expanded=False):
            st.code(extracted_text if extracted_text.strip() else "(empty)", language=None)
        ...
        if status in ("accepted", "rejected"):
            ...
            if st.session_state.get("active_review_editor") == review_id:
                ...
                with submit_col:
                    st.markdown('<div class="submit-accept-btn-marker"></div>', unsafe_allow_html=True)
                        ...
                        if not final_reason or final_reason == "Custom reason...":
                            ...
                            if not typed:
                                ...
                                try:
                                    ...
                                    # Index reviewed snippet to OpenSearch
                                    try:
...
def render_snippet_review_tab(config: Any) -> None:
    ...
    # ── Purge any missing snippet files from database ──
    try:
        purge_missing_reviews()
    except Exception as e:
        st.warning(f"Failed to purge missing reviews from database: {e}")
    ...
    with review_subtab:
        ...
        for snippet in filtered:
            if snippet.get("status") in ("accepted", "rejected"):
            ...
            if _is_obvious_noise_review_snippet(snippet, working_root):
                ...
                stale_status_updates.append((snippet["review_id"], "rejected", "Auto-rejected: Obvious noise snippet", "System Filter"))
                continue
            if _is_printed_font_review_snippet(snippet, working_root):
                hidden_printed_count += 1
                stale_status_updates.append((snippet["review_id"], "accepted", "Auto-accepted: Printed font (not handwriting)", "System Filter"))
                continue
            if _is_text_like_review_snippet(snippet, working_root):
                hidden_text_count += 1
                stale_status_updates.append((snippet["review_id"], "accepted", "Auto-accepted: OCR-readable text like region", "System Filter"))
                continue
            visible_snippets.append(snippet)
        filtered = visible_snippets

        if stale_status_updates:
            for rid, status, reason, actor in stale_status_updates:
                try:
                    update_snippet_review_status(rid, status=status, review_reason=reason, reviewed_by=actor)
                except Exception:
                    pass
            st.rerun()
