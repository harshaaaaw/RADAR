...
from core.redis_queue_manager import get_redis_queue_manager
...
def fix_statistics():
    ...
    try:
        ...
        # Diagnostics: inspect all docsearch:files:* keys
        logger.info("Running diagnostics on file keys...")
        file_keys = []
        cursor_f = 0
        while True:
            cursor_f, keys = r.scan(cursor_f, match="docsearch:files:*", count=1000)
            file_keys.extend(keys)
            if cursor_f == 0:
                break
        logger.info(f"Total docsearch:files:* keys found: {len(file_keys)}")
        if file_keys:
            # Inspect first 10 files
            for fk in file_keys[:10]:
                fields = r.hgetall(fk)
                logger.info(f"Key {fk} has fields: {fields}")
        
        # Bootstrap OCR completed and other counters from completion data
        logger.info("Running initialize_completed_counters bootstrap...")
        qm.initialize_completed_counters()
