...
def reset(force):
    ...
    try:
        ...
        # 10. Clear Visual Memory vectors (CNN feature vectors)
        click.echo("Clearing visual memory vectors...")
        visual_memory_dir = working_root.parent / "data" / "visual_memory"
        if visual_memory_dir.exists():
            import shutil
            try:
                shutil.rmtree(visual_memory_dir)
                visual_memory_dir.mkdir(parents=True, exist_ok=True)
                click.secho("✓ Cleared visual memory vectors", fg='green')
            except Exception as e:
                click.secho(f"  Warning: Could not clear visual memory: {e}", fg='yellow')
        else:
            click.echo("  No visual memory directory to clear")

        # 11. Clear Review Snippets (cropped images)
        click.echo("Clearing review snippets...")
        for snippet_dir in [working_root / "review_snippets", working_root.parent / "data" / "review_snippets"]:
            if snippet_dir.exists():
                import shutil
                try:
                    shutil.rmtree(snippet_dir)
                    snippet_dir.mkdir(parents=True, exist_ok=True)
                    click.secho(f"✓ Cleared review snippets: {snippet_dir}", fg='green')
                except Exception as e:
                    click.secho(f"  Warning: Could not clear {snippet_dir}: {e}", fg='yellow')

        # 12. Clear Redis backups
        click.echo("Clearing Redis backups...")
        redis_backup_dir = working_root.parent / "redis_backups"
        if redis_backup_dir.exists():
            backup_count = 0
            for backup_file in redis_backup_dir.iterdir():
                if backup_file.is_file():
                    try:
                        ...
                        pass
            if backup_count > 0:
                click.secho(f"✓ Cleared {backup_count} Redis backup files", fg='green')
            else:
                click.echo("  No Redis backup files to clear")
        else:
            click.echo("  No Redis backups directory to clear")

        # 13. Clear metadata uploads
        click.echo("Clearing metadata uploads...")
        metadata_dir = working_root / "metadata"
        if metadata_dir.exists():
            import shutil
            try:
                shutil.rmtree(metadata_dir)
                metadata_dir.mkdir(parents=True, exist_ok=True)
                click.secho("✓ Cleared metadata uploads", fg='green')
            except Exception as e:
                click.secho(f"  Warning: Could not clear metadata: {e}", fg='yellow')

        # 14. Write dashboard reset marker so surviving UI sessions hard-clear caches.
        try:
        ...
        click.echo("  • Visual memory vectors (.npy)")
        click.echo("  • Review snippet images")
        click.echo("  • Redis backups")
        click.echo("  • Metadata uploads")
...
def validate():
    ...
    try:
        ...
        required_packages = [
            'yaml', 'requests', 'opensearchpy', 'paddleocr', 'paddle',
