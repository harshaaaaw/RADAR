...
class ImagePreprocessor:
    ...
    def _preprocess_opencv_advanced(self, image_data: bytes, quality_score: float = None) -> Optional[bytes]:
        ...
        try:
            ...
            # This uses Hough line detection gross 90-degree correction and is configurable.
    ...
    def _correct_orientation(self, img: np.ndarray) -> np.ndarray:
        """Correct image orientation using Hough line detection (0, 90, 180, 270 degrees).

        PaddleOCR handles fine angle classification internally (cls=True), so this
        method only corrects gross 90-degree flips using OpenCV — no Tesseract needed.
        """
        try:
            if len(img.shape) == 3:
                gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            ...
            # Detect gross 90-degree rotations:
            # Hough theta for horizontal lines is ~90°, vertical ~0° or ~180°
            # If the majority of lines are near 0° or 180°, the document may be rotated 90°.
            near_vertical = sum(1 for a in angles if a < 20 or a > 160)
            near_horizontal = sum(1 for a in angles if 70 < a < 110)
            total = len(angles)

            if total > 0 and near_vertical / total > 0.7 and near_horizontal / total < 0.2:
                # Dominant vertical lines → likely rotated 90°
                logger.info("Detected likely 90° rotation via Hough lines — correcting.")
    ...
    def _correct_perspective(self, img: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            # Safety guard: only dewarp when the detected quadrilateral is likely
            # the outer page boundary. If it sits well inside the image, applying
            # perspective transform can crop away header/side content.
            pts_raw = doc_cnt.reshape(4, 2)
            h, w = gray.shape[:2]
            margin_x = int(w * 0.05)
            margin_y = int(h * 0.05)
            xs = pts_raw[:, 0]
            ys = pts_raw[:, 1]
            touches_edge = (
                np.min(xs) <= margin_x and
                np.max(xs) >= (w - margin_x) and
                np.min(ys) <= margin_y and
                np.max(ys) >= (h - margin_y)
            )
            if not touches_edge:
                return img
    ...
    def _remove_borders(self, img: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            left_margin = x
            right_margin = max(0, img_w - (x + w))
            top_margin = y
            bottom_margin = max(0, img_h - (y + h))

            # Safety guard: only perform border crop when margins are small.
            # Large crops are usually false positives that remove real text
            # near edges (especially headers/left-right columns).
            max_crop_ratio = 0.08
            if (
                left_margin > int(img_w * max_crop_ratio)
                or right_margin > int(img_w * max_crop_ratio)
                or top_margin > int(img_h * max_crop_ratio)
                or bottom_margin > int(img_h * max_crop_ratio)
            ):
                return img
