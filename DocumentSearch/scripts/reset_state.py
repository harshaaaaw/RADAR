...
def delete_queue_db(config) -> None:
    ...
    try:
        if queue_db_path.exists():
            ...
            logger.info(f"  ✓ Queue database deleted")
        else:
            logger.info(f"  ℹ Queue database not found (already clean)")
...
def main():
    ...
    if args.delete_index:
        ...
        logger.info(f"  • OpenSearch index: KEPT (use --delete-index to remove)")
